...
Selenium.prototype.doStore = function(varName, value) {
    chrome.runtime.sendMessage({ "storeStr": value, "storeVar": varName }).catch(() => {});
};

Selenium.prototype.doStoreText = function(locator, varName) {
    var element = this.browserbot.findElement(locator);
    chrome.runtime.sendMessage({ "storeStr": getText(element), "storeVar": varName }).catch(() => {});
};
...
Selenium.prototype.doStoreValue = function(locator, varName) {
    ...
    chrome.runtime.sendMessage({ "storeStr": this.getValue(locator), "storeVar": varName }).catch(() => {});
};

Selenium.prototype.doEcho = function(value) {
    chrome.runtime.sendMessage({ "echoStr": value }).catch(() => {});
};
...
Selenium.prototype.doPageWait = function() {
    ...
    window.sideex_page_done = expression();
};
// END
...
Selenium.prototype.doAjaxWait = function() {
    ...
    window.sideex_ajax_done = expression();
};
// END
...
Selenium.prototype.doWaitFor = function(locator, condition) {
    ...
    const result = this.browserbot.isElementClickable(element, {x, y});
...
Selenium.prototype.doType = function(locator, value) {
    ...
    if (element.type === 'range') {
        ...
        if (bot.dom.isEditable(element)) {
            ...
            throw new SeleniumError("Element is not accepting user input");
        }
    }
    // BZM_END
    // END
...
// BZM_BEGIN
Selenium.prototype.doTypeSecret = function(locator, value) {
...
Selenium.prototype.getEval = function(script) {
    ...
    return new Promise((resolve, reject) => {
        chrome.runtime.sendMessage({ type: 'evaluateScript', payload: script }).then(x => {
            let result = x;
            ...
            try {
                ...
                reject("Threw an exception: " + extractExceptionMessage(e));
...
Selenium.prototype.doResizeWindow = function (_url, windowSize) {
    ...
    if (window != null && window.screen) {
        ...
        chrome.runtime.sendMessage(messageOptions).catch(() => {});
...
Selenium.prototype.doRunScript = function(script) {
    ...
    return new Promise(resolve => {
        chrome.runtime.sendMessage({ type: 'executeScript', payload: script }).then(() => resolve()).catch(() => {});
    });
    ...
    // scriptTag.nonce = "nonce-deadbeef";
    //
...
Selenium.prototype.doEval = function(script) {
    return new Promise(resolve => {
        this.getEval(script).then(result => {
            if (result === 'null') {
                resolve(false);
            }
...
Selenium.prototype.doAssertEval = function(script) {
    return new Promise((resolve, reject) => {
        this.getEval(script).then(x => {
            ...
            if (!result) {
                reject("Assertion failed. Got result: " + result);
                // throw new Error();
            }
...
Selenium.prototype.doStoreEval = function(varName, script) {
    return new Promise(resolve => {
        this.getEval(script).then(x => {
            chrome.runtime.sendMessage({ "storeStr": x, "storeVar": varName }).catch(() => {});
            resolve();
        });
    });
};
...
Selenium.prototype.doFindFrame = function(locator) {
    const el = this.browserbot.findElement(locator);
    for (let i = 0; i < window.frames.length; ++i) {
        if (el.contentWindow === window.frames[i]) {
            return i;
        }
    }
