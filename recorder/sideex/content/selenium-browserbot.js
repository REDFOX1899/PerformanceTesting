...
var BrowserBot = function(topLevelApplicationWindow) {
    ...
    this.xpathEvaluator = new XPathEvaluator('javascript-xpath'); // change to "javascript-xpath" for the newer, faster engine
...
BrowserBot.prototype.findElementOrNull = function(locator, win) {
    ...
    let topWindow = win.top;
    // Find the element starting from the topmost window, recursively to all the child window frames.
    let element = this.findElementRecursive(locator.type, locator.string, topWindow.document, topWindow);
    // let element = this.findElementBy(locator.type, locator.string, win.document, win);
...
BrowserBot.prototype.locateElementByXPath.is_fuzzy_match = function(node, target) {
    return true;
};
...
BrowserBot.prototype.locateElementByLinkText.prefix = "linkText";
 ...
 *
 * @param formElement Form to submit
 */
BrowserBot.prototype.submitForm = function(formElement) {
